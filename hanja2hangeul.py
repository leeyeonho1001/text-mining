...
def hanja2hangeul_str2(str):
    ...
    for ch in str:
        if ch in h2h_table:
            hanja_word.append(ch)
            hanguel_word.append(h2h_table[ch][0])
            processing=True
        else:
            if processing==True:
                result.append(f'{"".join(hanguel_word)}({"".join(hanja_word)})')
                hanja_word=[]
                hanguel_word=[]
                processing=False
            else:
...
def hanja2hangeul_str3(str):
    ...
    for ch in str:
        if ch in h2h_table:
            ...
            if processing==True:
                result.append(f'{"".join(hanja_word)}({"".join(hanguel_word)})')
                hanja_word=[]
                hanguel_word=[]
                processing=False
            else:
